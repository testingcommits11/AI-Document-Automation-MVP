...
"""
PDF generation for demo documents and exported documents.

Field definitions come from the dynamic SQLite schema so newly added fields
automatically appear in generated PDFs.
"""
...
def build_demo_pdf(
    ...
    def draw_header():
        ...
        c.setFont("Helvetica-Bold", 16)
        c.drawString(60, top_y, cfg["doc_title"])

        c.setStrokeColorRGB(0.7, 0.7, 0.7)
        c.line(60, top_y - 12, width - 60, top_y - 12)
    ...
    for field in fields:
        # Start a new page when there is no room for another field.
        if y < bottom_y:
            c.showPage()
            draw_header()
            y = height - 120
        ...
        if field_key in filled_values:
            ...
            omit = negative and field_key == cfg.get("negative_field")
            if omit:
                value = ""
            else:
                value = str(cfg.get("demo_values", {}).get(field_key, ""))

        c.setFont("Helvetica-Bold", 12)
        c.setFillColorRGB(0, 0, 0)
        c.drawString(60, y, f"{field['label']}:")
    ...
    if filled_values:
        footer_label = "Updated Document with User Filled Data"
    elif negative:
        footer_label = "Fictional demo document — missing-field test"
    else:
        footer_label = "Fictional demo document"

    c.drawString(
        60,
        40,
        f"{footer_label} — AI Document Automation MVP",
    )
...
def build_combined_pdf(records: list) -> bytes:
    """
    Builds a combined PDF using the CURRENT dynamic field schema for each
    industry. Newly added fields therefore appear automatically.
    """
    ...
    def draw_header():
        ...
        c.drawString(
            60,
            top_y,
            "Processed Documents — Combined Summary",
        )
    ...
    for index, record in enumerate(records):
        ...
        # IMPORTANT:
        # Always retrieve the current dynamic fields from the database.
        fields = list_fields(industry_key)
        ...
        c.drawString(
            60,
            y,
            f"{index + 1}. {cfg['doc_title']} ({cfg['label']})",
        )
